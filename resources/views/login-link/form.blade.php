...
<head>
    ...
    <style>
        body { background: linear-gradient(135deg, #6366f1, #8b5cf6, #ec4899); }
    </style>
</head>
...
<body class="flex items-center justify-center min-h-screen px-4">
    ...
    <div class="w-full max-w-md">
        <div class="bg-white/90 backdrop-blur-md rounded-2xl shadow-2xl p-8">
            <div class="text-center mb-6">
                ...
                <p class="text-gray-500 text-sm mt-2">Enter your email to generate a secure login link</p>
            </div>

            @if(session('success'))
                <div class="bg-green-50 border border-green-200 rounded-lg p-4 mb-5">
                    <p class="text-green-700 text-sm font-medium mb-2">Login link generated successfully</p>
                    <div class="bg-white border rounded-md p-3 text-xs break-all text-gray-700" id="loginLink">
                        {{ session('success') }}
                    </div>
                    <button onclick="copyLink()" class="mt-3 w-full bg-green-600 text-white py-2 rounded-md hover:bg-green-700 transition">
                        Copy Login Link
                    </button>
                </div>
            @endif
            ...
            <form action="{{ route('login-link.send') }}" method="POST" class="space-y-4">
                ...
                <button type="submit" class="w-full bg-indigo-600 text-white py-3 rounded-lg font-semibold hover:bg-indigo-700 transition shadow">
    ...
    <script>
        function copyLink() {
            ...
            if (!linkBox) return;
            const link = linkBox.innerText.replace('Magic link generated successfully:', '').trim();
